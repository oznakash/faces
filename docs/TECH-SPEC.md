...
| `GET` | `/c/:slug/f/:face` · `/og/:face.jpg` · `/api/collections/:key/face/:face` | Share one person: page with Open Graph tags; the face as JPEG; face → current person |
...
**Sharing a person.** `/c/{slug}/f/{face}` is the share link for one person, where `{face}` is the first 12 hex characters of a *face* id — deliberately not a cluster id, because clusters are rebuilt with new ids on every regroup, while faces are permanent. The link resolves, at open time, to whichever person that face currently belongs to, so links survive adding a gallery (T-E12). The server serves the same page with Open Graph tags injected — `og:title` = collection name, `og:image` = that face as a 400×400 JPEG at `/og/{face}.jpg` (crawlers don't run JS, and several don't accept WebP), `twitter:card=summary` for the face-left layout — plus a `window.__FACES_OPEN` hint so the page opens that person immediately without a lookup. `GET /api/collections/:key/face/:face` is the same resolution for the client. The absolute URLs come from the proxy headers, or `FACES_PUBLIC_URL` when set.
